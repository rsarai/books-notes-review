...
- Create a copy of ``book_notes_review/settings/local.py.example``:
  `cp book_notes_review/settings/local.py.example book_notes_review/settings/local.py`
...
- Deregister the example app by removing `'exampleapp.apps.ExampleappConfig'` from ``backend/book_notes_review/settings/base.py``.
- Adjust ``backend/book_notes_review/urls.py`` to point to your newly created Django app and remove the path configuration that redirects to the deleted example app.
